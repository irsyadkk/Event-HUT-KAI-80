...
export default function WinnerDisplayPage() {
  ...
  const filtered = list.filter((w) => {
    ...
    return (
      String(w.winner || w.nipp || "").toLowerCase().includes(s) ||
  ...
  const stats = useMemo(() => {
    ...
    const gugur = filtered.filter((x) => String(x.status || "").toLowerCase() === "gugur").length;
    const belum = filtered.filter((x) => String(x.status || "").toLowerCase().includes("verifikasi")).length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-700 via-green-800 to-green-900" style={{ backgroundColor: "#406017" }}>
      <div className="p-6 space-y-8 max-w-6xl mx-auto">
        ...
        <div className="text-center py-8">
          ...
          <h1 className="text-5xl font-bold text-white mb-4 drop-shadow-lg">Pemenang Terpilih</h1>
          <p className="text-green-100">Hanya menampilkan NIPP pemenang, warna sesuai status.</p>
        </div>
        ...
        <div className="bg-white/95 rounded-2xl shadow-2xl border border-white/30 overflow-hidden">
          <div className="px-6 py-3 bg-white/60 text-sm text-gray-700 flex flex-wrap gap-4">
            <span className="inline-flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-green-600" /> Diambil</span>
            <span className="inline-flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-amber-500" /> Belum Verifikasi</span>
            <span className="inline-flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-red-600" /> Gugur</span>
            <span className="inline-flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-gray-400" /> Lainnya</span>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="p-6">
              {filtered.length ? (
                <div className="flex flex-wrap gap-2">
                  {filtered.map((w, i) => (
                    <span
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-bold shadow ${badgeClass(w.status)}`}
                      title={w.status || "-"}
                    >
                      {w.winner || w.nipp}
                    </span>
                ...
                <div className="py-10 text-center text-gray-500">Belum ada pemenang.</div>
              )}
            </div>
          )}
        </div>
