...
const AddMemberPage = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      if (orderMembersList.length > 0) {
        ...
        const firstOrderName = (orderMembersList[0]?.name ?? "")
          .trim()
          .toLowerCase();
  ...
  const submitOrder = async (e) => {
    ...
    if (uniqueNames.length === 0 && statusHadir === STATUS_TIDAK) {
      alert(
        "Harap masukkan setidaknya satu nama anggota atau ubah status pegawai menjadi Hadir."
      );
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Gagal mencetak tiket.";
  ...
  const handleAddMember = () => {
    ...
    if (currentCount < (maxMembers > 0 ? maxMembers - 1 : 0)) {
      ...
      alert(
        `Anda tidak bisa menambahkan lebih dari ${
          maxMembers - 1
        } anggota keluarga.`
      );
    }
